...
fuel_grid_generator <- function(aoi, aoi_buffer, lut, reference_grid, fuel_layers,parks,fuel_col,pc = F,pc_col, output_directory){
  
  aoi <- gBuffer(
            spTransform(
              aoi_poly[grep(parks,aoi_poly$parkname_e,ignore.case = T),],
              CRS(proj4string(reference_grid)
              )
            ),
            width = aoi_buffer
          )
  
  reference_grid <- crop(reference_grid,aoi)
  ...
  rsts <- lapply(rsts,function(x) {
    ...
    rst.in <- raster(x)
    rst.in <- extend(rst.in,raster(aoi,res=100,origin(reference_grid)))
    
    if(proj4string(rst.in) != proj4string(aoi)){
      
      rst.in <- projectRaster(rst.in,crs=CRS(proj4string(aoi)),method="ngb")
      rst.in <- crop(rst.in,raster(aoi,res=100,origin(reference_grid)),snap="in")
    ...
    if(res(rst.in)[1] != res(reference_grid)[1]){
      ...
      rst.in <- resample(rst.in,raster(aoi,res=100,origin(reference_grid)),method = "ngb")
